...
La segunda unidad consolida la base fonetica iniciada en Les1 y agrega herramientas comunicativas: pronombres personales (singular y plural, formal e informal), consonantes que suelen confundir (F/V/B/W, CH/SCH, NG, G), vocabulario tematico (colores avanzados, rutinas del dia, sentidos, salud) y expresiones corteses indispensables. El material se basa en las notas escritas de las cuatro partes de la clase y en las tareas 1-10 de la semana.
...
- Completar la serie de tareas 1-10 (pronombres, consonantes, vocabularios 4-6 y definiciones de tiempo).

## Pronombres personales en contexto

| Persona | Forma base | Variantes | Uso y notas |
|---------|------------|-----------|-------------|
| 1s | ik | - | Siempre minuscula; la K final se pronuncia. |
| 2s informal | je / jij | jij enfatiza a la persona | JE es neutro; JIJ se usa para recalcar "tu y no otro". |
| 2s formal | u | - | Se pronuncia como una I con labios redondeados. Funciona para singular y plural pero **se conjuga en singular**. |
| 3s masc | hij | ie (no acentuado) | IJ suena como AI. |
| 3s fem | zij | ze (no acentuado) | Igual regla de acento; ZE equivale a "ella" cuando no se enfatiza. |
| 3s neutro | het | 't (coloq.) | Al final de palabra suena casi como una X suave. |
| 1p | wij | we (no acentuado) | WIJ con dientes afuera; WE relajado. |
| 2p informal | jullie | - | Plural informal, equivalente a "ustedes/vosotros". |
| 3p | zij | ze | Mismo par que en singular; el contexto y el verbo indican si es singular o plural. |
...
3. Escribe tres oraciones usando `U` para un grupo (ej. instrucciones en la oficina) y comprueba que el verbo quede en singular.

## Cortesia rapida

- **Por favor / tenga**: `alsjeblieft` (informal) y `alstublieft` (formal). Se usa tanto para pedir como para entregar algo.
...
|--------|-----------------|--------------------|
| B / P | B inicial es cerrada (sin escape de aire). En posicion final suena como P. | `boek` (libro), `heb` -> [hep]. |
| F / V / B / W | V se pronuncia con dientes visibles; W holandesa suena entre U e I. | `vuur` (fuego), `buurt` (vecindario), `huur` (alquiler). |
| C + I/E | Suena como S. | `citroen` (limon). |
| NG | La G se vuelve muda; sonido nasal como en *English*. | `koning` (rey), `jongen` (chico), `lang` (largo). |
| CH y G | En palabras nativas suenan como una J fuerte; en prestamos franceses suenan "sh". | `wachten` (esperar), `chauffeur`, `douche`. |
...
- **DOEN** = actividad o procedimiento (`Ik doe de was`, `Ik doe boodschappen`, `Ik doe mijn huiswerk`). Habla de la accion en general.
- **MAKEN** = crear, reparar o materializar (`Ik maak soep`, `Ik maak mijn fiets`, `Ik maak een tekening`). Tambien se usa cuando te concentras en el resultado tangible.
- Algunas frases admiten ambas formas con matiz distinto: `Ik doe een examen` (participar en el examen) vs `Ik maak een examen` (resolverlo, construir la respuesta).
...
| Franja | Holandes | Comentarios |
|--------|----------|-------------|
| 06:00-12:00 | `de ochtend` o `de morgen` | En Belgica usan `voor 's ochtends` (06:00-09:30). |
| 12:00-18:00 | `de middag` | 12:00-14:00 = `vroeg in de middag`; 16:00-18:00 = `namiddag`. |
| 18:00-24:00 | `de avond` | Seis en punto ya cuenta como noche. |
| 24:00-06:00 | `de nacht` | Puntualidad: llegar 5 min antes es la norma. |

### Los cinco sentidos

| Sentido | Nombre | Organo | Verbo |
|---------|--------|--------|-------|
| Vista | `het gezichtsvermogen` | `de ogen` | `zien / kijken` |
| Oido | `het gehoor` | `de oren` | `horen / luisteren` |
| Olfato | `de reukzin` | `de neus` | `ruiken` |
| Gusto | `de smaakzin` | `de tong` | `proeven` |
| Tacto | `de tastzin` | `de huid` | `voelen` |

### Salud y sistema medico

- Primero se visita al `huisarts` (medico de cabecera). Sin referencia no te atienden en el hospital.
- Emergencias reales (huesos rotos, accidentes) van directo al hospital; de lo contrario el doctor indicara `uitzieken` (reposar) y usar paracetamol.
...
7. Tarea 7: aprender Vocabulario 4.
8. Tarea 8: aprender Vocabulario 5.
