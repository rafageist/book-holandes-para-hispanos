...
- Una sola consonante tras la vocal la deja "respirar": `maan` (luna) o `roken` (fumar).
- Dos consonantes cierran la vocal y la hacen corta: `man` (hombre), `rocken` (bailar rock).
- Al formar plurales o al conjugar verbos se mantiene la duracion original (`man` -> `mannen`, `raam` -> `ramen`).
- Comparacion con el espanol: en espanol la longitud vocal no cambia el significado (cama = llama). En neerlandes `man` y `maan` son palabras distintas, por ello siempre debes comprobar cuantas consonantes siguen a la vocal.

### 2. Vocal + R

Cuando una R sigue a EE u OO el timbre cambia. `eer` suena como "eir", `oor` mantiene un sonido abierto, `pier` no se lee "pir" sino "pie-eer". Repite lentamente y presta atencion a que la R siempre se pronuncia; no se omite como en espanol. Recuerda que la R final puede ser suave o fuerte segun la region, pero en el estandar neerlandes se escucha claramente.

#### Nota cultural

En Paises Bajos la R suele ser fuerte (casi como la de "perro" en espanol), mientras que en Surinam y Belgica algunas personas usan una R mas suave. No importa el estilo que elijas, siempre pronunciala para que no cambie el significado.
...
La R suele pronunciarse como en "raton". Segun la region puede suavizarse, pero en estas notas trabajamos con la version estandar del holandes central. Siempre cierra las palabras con su consonante final: `pop`, `bos`, `duck`. Esto te da puntos extra en examenes e interacciones reales. Recuerda que en neerlandes el acento escrito casi no se usa; cuando aparece (por ejemplo en `één` = uno) se coloca para evitar confusiones. Esta regla conecta con el subindice 1.6.

### 5. Pares minimos de referencia

| Contraste | Palabras nl | Traduccion | Pista rapida |
|-----------|-------------|------------|--------------|
| A corta vs AA | `man` / `maan` | hombre / luna | La AA se sostiene y se pronuncia con sonrisa. |
| O vs OO | `pop` / `poop`, `bos` / `boos` | muneca / munecas, bosque / enojado | Siente la diferencia al alargar la primera vocal. |
| O + R | `oor` / `oorlog` | oreja / guerra | Mantener la R evita que suene como "o". |
| U vs UI | `zus` / `huis` | hermana / casa | Usa el truco de "hamburguesa" para la U y labios redondeados para UI. |
| OE vs O | `boek` / `bok` | libro / macho cabrio | `oe` siempre suena como "u" inglesa. |
...
1. La luna es grande.  
2. Mi hermano tiene una casa azul.  
3. Escucho una historia corta.  
4. Ese libro es caro.  

Sugerencias de solucion:  
1. `De maan is groot.`  
2. `Mijn broer heeft een blauw huis.`  
3. `Ik hoor een kort verhaal.`  
4. `Dat boek is duur.`

### Completa los espacios

Rellena con la palabra correcta. Observa si necesitas vocal larga (doble) o corta.

1. `Ik heb twee ___ (maan/man) op de poster.`  
2. `De ___ (boos/bos) is donker in de herfst.`  
3. `Mijn ___ (zus/huis) woont in Utrecht.`  
4. `Wij lezen een ___ (boek/bok) in de les.`  
...
> `In de straat staat een groot huis. Voor het huis zit een buurman op de stoel. Hij leest de krant en hoort de vogels. Ik zeg hallo en hij lacht.`  
> Traduccion: En la calle hay una casa grande. Delante de la casa un vecino esta sentado en la silla. El lee el periodico y escucha a los pajaros. Yo digo hola y el rie.

### Dialogo modelo (con traduccion)

```
A: Hallo! Ik ben Marta. Hoe gaat het?
   (Hola, soy Marta. ¿Como estas?)
...
A: Prima. We lezen samen hardop.
   (Perfecto. Leemos juntos en voz alta.)
```
...
### Tabla rapida de colores y articulos

| Holandes | Espanol | Nota |
|----------|---------|------|
| `de tafel` | la mesa | Recuerda: `de` para la mayoria de palabras. |
...
1. `Het boek is rood.`  
2. `Ik schrijf mijn naam.`  

Traduce al neerlandes:

1. Mi hermana bebe agua.  
2. Nosotros vivimos en Utrecht.  

Soluciones sugeridas:  
1. El libro es rojo.  
2. Yo escribo mi nombre.  
...
Traduccion: Hoy practico la doble aa. Escribo "luna" y hago una frase. Tambien practico ui y escribo "casa".

## Mini dialogo sugerido

> **A:** Hallo! Ik ben Marta. Hoe gaat het?  
> **B:** Goed, dank je. Ik oefen de vocalen. Ik zeg *bos*, *baas*, *boos*.  
> **A:** Welke woorden zijn moeilijk?  
> **B:** De maan en de man. Ik schrijf het nu op.  
...
- **Tarea 8**: memorizar el vocabulario de la fruta y usarlo en oraciones simples.
...
1. Confundir `ui` con `oe`: recuerda que `ui` se pronuncia como una mezcla de i y u con labios redondeados.
2. Traducir articulos literalmente: no digas `el huis`; usa `het huis`.
3. Olvidar consonantes finales: `bos` (bosque) necesita la S final para no convertirse en otra palabra.
